# AliasKit — MVP Specification

## Vision

One API call, one complete digital identity for your AI agent. AliasKit provisions email inboxes and phone numbers so agents can operate as independent entities on the internet — no borrowed credentials, no manual setup.

## Product Positioning

AgentMail (YC S25) solved email. AgentPhone is emerging for phone. AliasKit bundles both into a single identity primitive: one call creates a coherent identity (name, email, phone) that an agent can use to sign up for services, receive verifications, and communicate autonomously.
...
1. SMS arrives at Plivo number
2. Plivo sends webhook to `/api/internal/sms/inbound`
...
2. API route calls Plivo API to search available numbers
3. Purchases number, configures SMS webhook URL
4. Stores number + Plivo ID in identity record
...
console.log(identity.phoneNumber)  // +12025551234

// Read inbox
const emails = await ak.identities.emails(identity.id, { unread: true })
const code = emails.data[0].body_text.match(/\d{6}/)?.[0]

// Read SMS
const sms = await ak.identities.sms(identity.id, { unread: true })
const smsCode = sms.data[0].body.match(/\d{6}/)?.[0]
...
SDK is a thin wrapper over fetch — no dependencies, works in Node, Deno, Bun, browsers.
...
AliasKit's differentiator: **bundled identity**, not just individual channels. One API call, one coherent person.

---

## Success Metrics (MVP)

- Can create an identity with email + phone in < 5 seconds
- Can receive a verification email and extract the code via API
- Can receive a verification SMS and extract the code via API
- Can send an email from the provisioned address
- Dashboard shows all identities and their inboxes
- SDK works end-to-end in a Node.js script
- 10 beta users from waitlist actively using the API
